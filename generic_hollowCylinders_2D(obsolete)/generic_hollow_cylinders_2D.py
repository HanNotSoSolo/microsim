...
class ForceOnTwoParallelCylinders:
    ...
    def postprocess_force(self, postprocess_file_R1, postprocess_file_R2,
        ...
        if getYukawa:
            ...
            param_R1.set_data(postprocess_file_R1.sol_int + image_R2)
            ...
            force_cylinder_1_R1 = -wf_R1.pb_cst.evaluate(expression_cylinder_1_R1,
                                                         var_dict={'param': param_R1}) * rho_1

            force_cylinder_1_R1 *= [2 * np.pi, 2 * np.pi]
            ...
            pseudo_coors_int_R1 = postprocess_file_R2.coors_int - np.array([self.R_1, self.Z_1])
            image_R1 = postprocess_file_R1.evaluate_at(pseudo_coors_int_R1,
                                                       mode='val')
            param_R2.set_data((postprocess_file_R2.coors_int[:, 0] * postprocess_file_R2.sol_int) + (pseudo_coors_int_R1[:, 0] * image_R1))

            expression_cylinder_2_R2 = "ev_grad.{}.subomega301(param)".format(wf_R2.integral.order)

            force_cylinder_2_R2 = -wf_R2.pb_cst.evaluate(expression_cylinder_2_R2,
                                                         var_dict={'param': param_R2}) * rho_2

            force_cylinder_2_R2 *= [2 * np.pi, 2 * np.pi]
            ...
            print("Force on cylinder 1:", force_cylinder_1_R1)
            print("Force on cylinder 2:", force_cylinder_2_R2)
...
def test():
    ...
    ''' === ANALYTICAL COMPUTATION (debug) === '''
    ana_c1 = cylinder(radius=R_ext_1, height=h_1,
                      density=rho_1, innerRadius=R_int_1)
    ana_c2 = cylinder(radius=R_ext_2, height=h_2,
                      density=rho_2, innerRadius=R_int_2)

    # Creating the interaction between the two cylinders
    c1_c2 = cylinderPair(ana_c1, ana_c2)

    # Computing the force on the Z axis of the cylinder
    ana_Fx_1 = c1_c2.cmpAnaFx([R_1, 0, Z_1], [0, 0, 0],
                            _dir='2->1')
    ana_Fz_1 = c1_c2.cmpAnaFz([R_1, 0, Z_1], [0, 0, 0],
                            _dir='2->1')

    print("\nForce on cylinder 1 (analytical): [", ana_Fx_1, ana_Fz_1, "]")

    ''' === END OF ANALYTICAL COMPUTATION (debug) ==='''
